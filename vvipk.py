...
def display_reaction_types():
    reaction_types = {
        ...
        '5': 'SAD',
        '6': 'ANGRY',
        '7': 'MIX'  # New mixed reaction option
    }
...
def mix_reaction(filename, post_id, reaction_type, reaction_count, delay):
    def process_reaction(i, access_token, reaction_type):
        ...
        # Handle mixed reactions
        if reaction_type == 'MIX':
            reaction_choices = ['LIKE', 'SAD']
            reaction_type = random.choice(reaction_choices)

        params = {'access_token': access_token, 'type': reaction_type}
        handle_request(url, params, headers, filename, i, 
                       f"{G}[SUCCESS] ----- {Y} SUCCESSFULLY REACTED ON POST", 
                       f"{R}[FAILED] ----- {R} FAILED TO REACT ON POST", 
                       user_id=None, post_id=post_id)
        time.sleep(delay)
    ...
    # Normalize reaction_type for validation
    available_reactions = ['LIKE', 'LOVE', 'HAHA', 'WOW', 'SAD', 'ANGRY', 'MIX']
    if reaction_type.upper() in available_reactions:
        reaction_type = reaction_type.upper()
    else:
        # Explicitly react with LIKE, LOVE, and WOW
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i in range(reaction_count):
            ...
            # Use the specific reactions in a round-robin manner
            reaction_to_use = reaction_type[i % len(reaction_type)]
            future = executor.submit(process_reaction, i, access_token, reaction_to_use)
            futures.append(future)
...
def boosting_menu():
    while True:
        ...
        rp(pan(f"{Y}[1] {G}AUTO REACT    [PAGE & NORMAL]\n"
                ...
                f"{Y}[6] {G}REPLY COMMENT [PAGE & NORMAL]\n"
                f"{Y}[7] {G}AUTO SHARE    [PAGE & NORMAL]\n"
